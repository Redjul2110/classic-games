...
export function renderPartyNeonCards(container, onBack) {
    ...
    channel.on('broadcast', { event: 'request_sync' }, () => {
        if (isHost && state !== 'waiting') {
            broadcastState(); // Host resends state
        }
    });
    ...
    function broadcastState(msg) {
        ...
        else state = 'playing'; // <= Ensure host local state transitions to playing!
    ...
    if (!isHost) {
        ...
        }, 1500); // Ask host for state if we haven't received it yet
    }
