...
export function renderPartyDraw(container, onBack) {
        ...
        <div class="game-screen" style="max-width:960px;margin:0 auto;display:flex;flex-direction:column;height:100vh;">
            ...
            <div style="flex:1;display:flex;flex-wrap:wrap;gap:12px;padding:12px;overflow:auto;align-items:flex-start;">
                ...
                <div style="width:min(100%,220px);display:flex;flex-direction:column;gap:8px;">
                    ...
                    <div style="background:var(--bg-card);border-radius:12px;padding:12px;border:1px solid rgba(255,255,255,0.1);flex:1;min-height:120px;display:flex;flex-direction:column;">
    ...
    channel.on('broadcast', { event: 'draw_guess' }, ({ payload }) => {
        ...
        // ── Security: never trust the client-sent `correct` flag.
        //    Recompute locally from our own currentWord so no one can spoof points.
        const correct = !!(currentWord && guess && guess.trim().toLowerCase() === currentWord.toLowerCase());
        appendGuess(username, correct ? '✅ Got it!' : guess, correct);
        if (correct) {
            // Award points: more points the earlier you guess
            ...
            if (currentDrawerUid && currentDrawerUid !== uid) {
                totalScores[currentDrawerUid] = (totalScores[currentDrawerUid] || 0) + 5;
            }
            if (uid === myId) {
            ...
            if (isHost && correctGuessers.size >= uids.length - 1) {
                if (roundTimer) clearInterval(roundTimer);
                channel.send({ type: 'broadcast', event: 'draw_round_end', payload: { word: currentWord, nextRound: currentRound + 1 } });
                handleRoundEnd(currentWord, currentRound + 1);
            }
    ...
    function submitGuess() {
        ...
        if (!raw) return;
        guessInput.value = '';
        // Only send the raw guess — receivers compute correctness themselves.
        channel.send({ type: 'broadcast', event: 'draw_guess', payload: { uid: myId, username: members[myId]?.username || 'Me', guess: raw } });
    }
